...
class App extends BaseComponent<Props, State> {
	...
	render(): JSX.Element {
		...
		const { router, children, user } = this.props;	  
	...
	renderTopBar() {
		const { layout, page, isNavBarCollapsed } = this.state;
	...
	renderLeftBar() {
		const { layout, isNavBarCollapsed } = this.state;
		const { user } = this.props;
